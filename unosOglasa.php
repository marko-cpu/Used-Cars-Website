...
<div class="image-upload">
    <label for="file-input">
        <img id="preview-image" style="margin-left:auto; margin-right:auto; width:50%" src="https://icons.iconarchive.com/icons/paomedia/small-n-flat/512/cloud-up-icon.png"/>
    </label>
    <input id="file-input" type="file" name="file[]" multiple required onchange="previewImages(event)">
</div>
...
<script>
    function previewImages(event) {
        var files = event.target.files; // Dobijanje niza fajlova koje je korisnik izabrao
        var imageContainer = document.getElementById('preview-image'); // Dohvatanje elementa za prikaz trenutno izabrane slike
        var imageListContainer = document.getElementById('image-list'); // Dohvatanje elementa za prikaz liste slika

        // Provera da li je izabrana barem jedna slika
        if (files && files.length > 0) {
            imageListContainer.innerHTML = ""; // Očisti prethodni prikaz liste slika

            // Prolazak kroz sve izabrane slike
            for (var i = 0; i < files.length; i++) {
                var reader = new FileReader(); // Kreiranje FileReader objekta

                // Postavljanje funkcije koja će se izvršiti kada se slika učita
                reader.onload = (function (file) {
                    return function (e) {
                        var imageElement = document.createElement('img'); // Kreiranje novog elementa slike
                        imageElement.style.width = '100px'; // Postavljanje veličine slike
                        imageElement.src = e.target.result; // Postavljanje izabranog fajla kao izvora slike

                        imageListContainer.appendChild(imageElement); // Dodavanje slike u listu slika
                    };
                })(files[i]);

                reader.readAsDataURL(files[i]); // Čitanje izabranog fajla kao URL-a slike
            }
        }
    }
</script>
